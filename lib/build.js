...
module.exports = {
		...
		files.forEach((file) => {
			...
			if ("_global" in d) {
				if (typeof d._global == "function") {
					merge(config, d._global.call(config));
				} else {
			...
			if (system in d) {
				if (typeof d[system] == "function") {
					merge(config, d[system].call(config));
				} else {
			...
			if (isLocal === 'true' && "_local" in d) {
				if (typeof d._local == "function") {
					merge(config, d._local.call(config));
				} else {
	...
	findConfigFiles: function(directory) {
		return findParentFiles(directory, "leo_config.js").reverse();
	},
	dynamicBuild: function(directory) {
		return this.build(this.findConfigFiles(directory));
	}
};
